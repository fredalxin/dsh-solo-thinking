...
function endSpace(space, now) {
	if (space.endedAt !== void 0) throw new Error("Thinking space has already ended");
	return {
		...space,
...
function apply(ctx, rawConfig = {}) {
	...
	const endThinking = async (agent) => {
		const initial = requireSpace(ctx, agent);
		return serializeWrite(writes, initial.space.rootSessionId, async () => {
			const ended = endSpace(requireSpace(ctx, agent).space, Date.now());
	...
	ctx.inject(["sessionProjections"], (projectionCtx) => projectionCtx.sessionProjections.register({
		...
		view: (state) => state?.endedAt === void 0 ? state : null,
		stateVersion: 1
	}));
...
function locateSpace(ctx, sessionId) {
	...
	for (const session of ctx.sessions.list()) {
		...
		if (!space || space.endedAt !== void 0) continue;
