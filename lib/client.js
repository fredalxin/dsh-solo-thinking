...
const BETTER_SIDEBAR_TAB_ID = "dsh-plugin-solo-thinking:tree";
...
function watchBetterSidebar(sessions, sidebar) {
	let currentSessionId;
	let disposeProjection;
	const openedSessions = /* @__PURE__ */ new Set();
	const bindCurrent = () => {
		...
		const openWhenReady = () => {
			const space = projection.getSnapshot();
			if (!space) return;
			const snapshot = sessions.list.getSnapshot();
			const targets = [...space.nodes.filter((node) => node.sessionId !== sessionId), ...space.nodes.filter((node) => node.sessionId === sessionId)];
			for (const node of targets) {
				const key = `${node.sessionId}:${space.rootSessionId}`;
				if (openedSessions.has(key)) continue;
				openedSessions.add(key);
				const cwd = snapshot.byId[node.sessionId]?.cwd;
				sidebar.openTab({ type: BETTER_SIDEBAR_TAB_ID }, cwd ? {
...
function createBranchActions(sessions) {
	return {
		openSession: (sessionId) => openSessionInConversation(sessions, sessionId),
...
const CHAT_VIEW_LABELS = /* @__PURE__ */ new Set(["对话", "Chat"]);
function activateConversationView(root = document) {
	const tab = Array.from(root.querySelectorAll("button[role=\"tab\"]")).find((candidate) => CHAT_VIEW_LABELS.has(candidate.textContent?.trim() ?? ""));
	if (!tab) return false;
	if (tab.getAttribute("aria-selected") !== "true") tab.click();
	return true;
}
function shouldOpenNodeConversation(node, currentSessionId, state) {
	const dormant = node.dormant ?? state?.blank ?? false;
	return node.sessionId !== currentSessionId && !dormant && !node.forkHandoffPending;
}
function openSessionInConversation(sessions, sessionId) {
	sessions.open(sessionId);
	if (typeof document === "undefined" || typeof requestAnimationFrame === "undefined") return;
	let attempts = 0;
	const activate = () => {
		attempts += 1;
		if (sessions.list.getSnapshot().current === sessionId && activateConversationView()) return;
		if (attempts < 10) requestAnimationFrame(activate);
	};
	requestAnimationFrame(activate);
}
function useObservableSnapshot(source) {
	return (0, react.useSyncExternalStore)((listener) => source.subscribe(listener), () => source.getSnapshot(), () => source.getSnapshot());
}
function BetterSidebarThinkingTab({ ctx, scope, visible }) {
	if (!visible) return null;
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(BetterSidebarThinkingTabVisible, {
		ctx,
		sessionId: scope.sessionId
	});
}
function BetterSidebarThinkingTabVisible({ ctx, sessionId }) {
	const sessions = ctx.sessions;
	const useSessions = ((selector) => selector(useObservableSnapshot(sessions.list)));
	const useProjection = ((key, selector) => {
		const value = useObservableSnapshot(sessions.binding(sessionId)?.session.projections.faceOf(key) ?? absentProjection);
		return selector ? selector(value) : value;
	});
	if (!useProjection("soloThinking")) return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
		className: "str-empty",
		"aria-label": "头脑风暴尚未开始",
		children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("style", { children: railStyles }),
			renderThinkingTreeIcon(34),
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: "这个会话还没有头脑风暴" }),
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: "在对话中让 Agent 开启头脑风暴后，思考树会自动出现在这里。" })
		]
	});
	const props = {
		sessionId,
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ThinkingRail, { ...props });
}
function renderThinkingTreeIcon(size) {
...
function ThinkingView({ sessionId, useProjection, useSession, useSessions, openSession, sendToBranch, runCommand }) {
	...
	const submitAction = async (event) => {
		...
		setError(null);
		try {
			const line = action === "return" || action === "checkpoint" || action === "end" ? `/thinking ${action}` : `/thinking ${action} ${JSON.stringify({ title })}`;
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
					...
					children: [
						...
						action && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("form", {
							...
							children: [
								...
								action === "end" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									className: "st-return-explain",
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: "结束后整棵树会从界面清空。" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: "历史 Session 和 Handoff 仍会保留；任一原分支会话都可以重新开启一棵独立的新树。" })]
								}),
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(FullBranchContextCard, {
							eyebrow: "兄弟感知",
...
function ThinkingRail({ sessionId, useProjection, useSessions, openSession, sendToBranch, runCommand, openDetails, autoOpen = true }) {
	const space = useProjection("soloThinking");
	const layout = (0, react.useMemo)(() => space ? computeOrbitLayout(space) : null, [space]);
	const current = space?.nodes.find((node) => node.sessionId === sessionId);
	const [selectedNodeId, setSelectedNodeId] = (0, react.useState)(null);
	...
	const selectNode = (node) => {
		...
		setSplitOpen(false);
		setError(null);
		if (shouldOpenNodeConversation(node, sessionId, sessionStates[node.sessionId])) openSession(node.sessionId);
	};
	const send = async () => {
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("svg", {
					...
					return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						}), selectedPoint && canControl && !selectedRunning && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: selected.parentId ? "已选分支" : "主方向" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", { children: selected.title })] }), !selectedIsCurrent && !selectedDormant && !selected.forkHandoffPending && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							onClick: () => openSession(selected.sessionId),
							children: "进入对话 ↗"
						})]
					...
					canControl && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							!selected.parentId && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								disabled: controlling || selectedRunning,
								onClick: () => globalThis.confirm("结束整棵头脑风暴？历史会话会保留。") && void control("/thinking end"),
								children: "■ 结束"
							})
						]
					}),
					...
					selectedIsCurrent ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						...
						}, selected.id), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: "Enter 发送 · Shift+Enter 换行" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "str-context-index",
						"aria-label": "分支上下文",
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(RailContextSection, {
								eyebrow: "当前结论",
								title: selectedDormant && selected.status === "active" ? "未启动" : statusLabel(selected),
								meta: selected.returnedHandoff ? "最终回传" : selected.checkpointHandoff ? "阶段进展" : "尚未发布",
								tone: nodeTone(selected),
								children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(HandoffMarkdown, { text: selected.returnedHandoff ?? selected.checkpointHandoff ?? (selectedDormant ? "这个方向尚未启动，因此还没有对外发布结论。" : "这个方向尚未发布 Current State。完成一次有意义的讨论后，Agent 会自动整理可共享结论。") })
							}, `current:${selected.id}`),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(RailContextSection, {
								eyebrow: "父节点结论",
								title: parent?.title ?? "根节点",
								meta: parent ? selected.inheritedHandoff ? "已继承" : selected.forkHandoffPending ? "准备中" : "无额外交接" : "思考起点",
								children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(HandoffMarkdown, { text: selected.inheritedHandoff ?? (selected.forkHandoffPending ? "父 Agent 正在准备这个分支专属的继承 Handoff。完成前不会复制或暴露父分支原始对话。" : parent ? "父分支没有为这里制造额外上下文。" : "这是整棵思考树的起点，没有父节点上下文。") })
							}, `parent:${selected.id}`),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(RailContextSection, {
								eyebrow: "兄弟感知",
								title: siblings.length ? `${siblings.length} 个同级方向` : "没有同级方向",
								meta: siblings.length ? `${publishedSiblings}/${siblings.length} 已发布` : "无",
								children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(RailBranchContextList, {
									nodes: siblings,
									kind: "sibling",
									onSelect: selectNode
								})
							}, `siblings:${selected.id}`),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(RailContextSection, {
								eyebrow: "子节点结论",
								title: children.length ? `${children.length} 个子分支` : "还没有子分支",
								meta: children.length ? `${returnedChildren}/${children.length} 已回传` : "无",
...
function RailContextSection({ eyebrow, title, meta, defaultOpen = false, tone = "active", children }) {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("details", {
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("summary", { children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: eyebrow }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: title })] }),
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", { children: meta }),
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("i", {
				"aria-hidden": "true",
				children: "⌄"
			})
		] }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
			className: "str-context-content",
			children
		})]
	});
}
function RailBranchContextList({ nodes, kind, onSelect }) {
	if (nodes.length === 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		className: "str-context-empty",
		children: kind === "sibling" ? "当前节点没有兄弟分支。" : "从图中“＋”可以创建一个子分支。"
	});
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		className: "str-context-list",
		children: nodes.map((node) => {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("article", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
				type: "button",
				onClick: () => onSelect(node),
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { className: `st-dot st-dot--${nodeTone(node)}` }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: node.title }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", { children: phase })] }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("b", { children: "↗" })
				]
			}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(HandoffMarkdown, {
				compact: true,
...
function HandoffMarkdown({ text, compact = false }) {
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		className: `st-markdown${compact ? " is-compact" : ""}`,
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
		...
		children: [
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(HandoffMarkdown, { text: body })
			})
		]
	});
}
function FullBranchContextCard({ eyebrow, title, meta, nodes, kind, onSelect }) {
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
		className: "st-related st-related-context",
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", {
			className: "st-related-heading",
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
				className: "st-kicker",
				children: eyebrow
			}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("h2", { children: title })] }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", { children: meta })]
		}), nodes.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
			className: "st-muted",
			children: kind === "sibling" ? "当前节点没有兄弟分支。" : "还没有子分支；可以从图上的“＋”创建。"
		}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
			className: "st-related-list",
			children: nodes.map((node) => {
				const handoff = node.returnedHandoff ?? node.checkpointHandoff;
				const phase = node.returnedHandoff ? "最终回传" : node.checkpointHandoff ? "阶段进展" : statusLabel(node);
				return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("article", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", { children: phase }),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("b", { children: "↗" })
					]
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: "st-related-handoff",
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(HandoffMarkdown, {
						compact: true,
						text: handoff ?? (kind === "sibling" ? "尚未发布可供兄弟分支读取的结论。" : "尚未向当前节点回传最终结论。")
					})
				})] }, node.id);
...
`;
//#endregion
exports.BETTER_SIDEBAR_TAB_ID = BETTER_SIDEBAR_TAB_ID;
...
exports.shouldOpenNodeConversation = shouldOpenNodeConversation;
