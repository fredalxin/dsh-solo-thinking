...
成功输出包含建议/根/子 Session ID、`suggestedBranches: 4`、`suggestedBranchesDormant: true`、`workspaceInherited: true`、生命周期树的 `revision: 7`、两个 `agent-authored` Handoff 标记、`childStatus: "returned"`、`parentNotice: true`、`childSessionPreserved: true` 和 `restartAfterEnd: true`。脚本同时断言：
...
- `/thinking end` 清空当前 Projection 后，同一个 Session 可以从 revision 0 新建独立思考树；
